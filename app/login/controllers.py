...
class UserLogin(MethodView):  #/login
    ...
    def post(self): 
        ...
        if administrador and administrador.verify_password(dados['password']): 
            token = create_access_token(identity=administrador.id, expires_delta=timedelta(minutes=900), fresh=True, additional_claims={"is_administrator": True})
            ...
            if advogado and advogado.verify_password(dados['password']):
                token = create_access_token(identity=advogado.id, expires_delta=timedelta(minutes=900), fresh=True, additional_claims={"is_advogado": True})
